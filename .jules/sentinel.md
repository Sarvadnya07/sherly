...
- Always use constant-time comparison methods like `secrets.compare_digest` from the built-in `secrets` module when verifying security credentials, tokens, or API keys.## 2024-08-02 - Hardcoded API Key in Frontend UI\n**Vulnerability:** The frontend user interface (`remote_ui/index.html`) had a hardcoded API key (`"sherly123"`) embedded directly in the JavaScript source code.\n**Learning:** Hardcoding API keys in frontend, client-side code exposes the key to any user who inspects the source, allowing unauthorized access to the backend API.\n**Prevention:** API keys should be dynamically provided by the user (e.g., via a prompt) and stored securely on the client-side (e.g., in `localStorage`) rather than being hardcoded in the source file.
