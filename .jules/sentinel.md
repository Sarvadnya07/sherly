...
## 2024-05-30 - Command Injection via Unvalidated System Commands in Agent
**Vulnerability:** The System Agent (`agents/system_agent.py`) executed arbitrary OS commands via the raw `run_command` function without validation when interpreting LLM outputs.
**Learning:** System automation agents that execute parsed LLM output or user input as shell commands must always use a central validation gateway (whitelist/safety guard). Direct calls to raw execution functions bypass security layers and are highly vulnerable to prompt injection or command injection.
**Prevention:** Always use `safe_exec` from `tools.terminal_tools` instead of `run_command` when dynamically executing shell commands from external inputs or LLM generation.
