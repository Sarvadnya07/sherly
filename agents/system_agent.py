...
def run(prompt: str, ask_model=None) -> str:
    # Sentinel Security Fix: Replaced raw run_command with safe_exec
    # to ensure all shell commands are validated against the whitelist and safety guard,
    # preventing command injection from unvalidated LLM output or user input.
    if not ask_model:
