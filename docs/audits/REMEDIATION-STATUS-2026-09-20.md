...
|----|--------------------|------|------------------------|
| ARCH-001 (+ SEC-006, TEST-001) | Session/client identity did not exist; approvals/confirmations were process-global. **RESOLVED 2026-09-20** — see below. Remaining: `preview_store` session binding. | MEDIUM — single-user local app; risk materializes with remote PWA concurrent use | Delivered in `approval_service.py` per `REMEDIATION-STATUS-SESSION-APPROVALS-2026-09-20.md` | FIXED (preview-store follow-up remains) |
