...
| SEC-006 | Global single-slot confirmation state | STILL_VALID | not addressed this pass (needs session identity — ARCH-001) | — | — | — | **DEFERRED** |
...
| TEST-001 | No client-isolation tests | STILL_VALID | blocked by ARCH-001 (cannot test a property that doesn't exist) | — | — | — | **DEFERRED** |

## DEFERRED items — reasons and required next phase

| ID | Reason for deferral | Risk | Recommended next phase |
|----|--------------------|------|------------------------|
| ARCH-001 (+ SEC-006, TEST-001) | Session/client identity does not exist anywhere in the system; threading one through 4 stores is a cross-cutting design change touching API, WS, frontend and every store. Half-fixing it (e.g. keying only `_pending_actions`) would create a false sense of isolation while `_pending_confirmation` and `preview_store` remain global. | MEDIUM — single-user local app; risk materializes with remote PWA concurrent use | Dedicated design pass: introduce a session ID issued by the backend, thread it through WS connect + REST, key all four stores by it, then add TEST-001 isolation suite |
