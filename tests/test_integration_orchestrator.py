...
def test_execute_objective(orchestrator):
    with patch("sherly.core.orchestrator.log") as mock_log:
        ...
        # Since MockAgent returns "Result for <prompt>" and the Orchestrator falls back
        # to a default task when parsing the MockAgent's non-JSON response:
        assert "Analyze and implement: Test Objective" in result
