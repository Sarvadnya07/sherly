...
def test_ask_model_timeout_fallback(mock_post):
    ...
    with patch("model_manager.search_web") as mock_search:
        ...
        with patch("model_manager.get_current_model", return_value="openai"):
            with patch("model_manager.get_api_key", return_value="test_key"):
...
def test_ask_model_http_error(mock_post):
    ...
    with patch("model_manager.get_current_model", return_value="openai"):
        with patch("model_manager.get_api_key", return_value="test_key"):
            with patch("model_manager.search_web", return_value=[]):
                result = ask_model("Tell me a joke")
